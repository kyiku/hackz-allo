...
**ユーザーストーリー:** 開発者として、ステータス画面で装備やAIの戦い方・編成を調整し、自分の状態を把握したい。RPGのパーティ編成のように、AIの能力を組み替えて戦いに臨みたい。

- THE SYSTEM SHALL ステータス画面で、獲得済みスキル/MCPをスロットに装備・解除できるようにする
- WHEN プレイヤーが装備を変更する THE SYSTEM SHALL 次回以降の戦闘のAIエージェント実行設定（使用ツール/MCP/スキル）に反映する
